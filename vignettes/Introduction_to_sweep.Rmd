...
We'll use the `tidyquant` package to get the US alcohol sales, which comes from the FRED data base (the origin is the US Bureau of the Census, one of the 80+ data sources FRED connects to). The FRED code is "S4248SM144NCEN" and the data can be found [here](https://fred.stlouisfed.org/series/S4248SM144NCEN).
...
We can do this a few ways. First, we'll take a look at the current way using `stats::ts()`. 
...
We can actually get the index using `sw_index()`.
...
The modeling workflow takes a time series object and applies a model. Nothing new here. Just use the `forecast` package. We'll use the `ets()` function to get an ETS (Error, Trend, Seasonal) model.
...
Going through the tidiers, we can get useful `model` information. `sw_tidy()` returns the model parameters.
...
We can review the decomposition using `ggplot2` as well.
