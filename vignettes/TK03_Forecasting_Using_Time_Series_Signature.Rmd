...
Visualize the residuals of the test set. Looks like there may be a slight pattern left as we get closer to the end of 2012. 
...
We can also review the RMSE of the model to get an idea of test set error. The test error is higher than the training error, which is always the case.

```{r}
# RMSE
sqrt(mean((pred_test$.resid)^2))
```

At this point you might go back to the model and try tweaking to features, adding other features that may be known in the future (e.g. temperature of day can be forecasted relatively accurately within 7 days), or try a completely different modeling technique (e.g. SVM regression) with the hope of better predictions on the test set. Once you feel that your model is optimized, move on the final step of forecasting. 

# Forecasting

What are the expected future values for the next six months? Let's use our model to predict this.

The first step is to create the date sequence. Let's use `tk_get_timeseries_summary()` to review the summary of the dates from the original dataset, "bikes". 
...
bikes %>%
    ...
    geom_rect(xmin = as.numeric(ymd("2013-01-01")),
              ...
              ymin = 0, ymax = 10000,
              fill = palette_light()[[9]], alpha = 0.01) +
    annotate("text", x = ymd("2011-10-01"), y = 7800,
             color = palette_light()[[1]], label = "Train Region") +
    annotate("text", x = ymd("2012-10-01"), y = 1550,
             color = palette_light()[[1]], label = "Test Region") +
...
Note that there are no error bands. I'm still researching suitable techniques to include confidence intervals for forecasting using this approach.
