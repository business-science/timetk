...
test_that("mts to tbl test returns tibble with correct rows and columns.", {
    ...
    # Warning if no sweep index attribute
    expect_warning(
        WWWusage %>%
...
test_that("forecast::msts to tbl test returns tibble with correct rows and columns.", {
    ...
    expect_warning(
        sw_tbl(test_default, preserve_index = T, index_rename = "index")
    )
    test_tbl_9 <- sw_tbl(test_default, preserve_index = F, index_rename = "index")
